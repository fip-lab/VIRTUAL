...
def get_input_1(tokenizer, max_length, example):
    max_segments = 1
    ...
    segments = []
    encoded_option_evidence = tokenizer(question + '[SEP]' + '[SEP]'.join(options) + '[SEP]' + passage,
                                        padding="max_length",
                                        max_length=max_length,
                                        truncation=True,
                                        add_special_tokens=True)
    segment = {}
    segment['input_ids'] = encoded_option_evidence['input_ids']
    segment['attention_mask'] = encoded_option_evidence['attention_mask']
    segments.append(segment)

    return (segments)


def get_input_21(tokenizer, max_length, example):
    pass

def get_input_22(tokenizer, max_length, example):
    pass

def get_input_31(tokenizer, max_length, example):
    pass

def get_input_32(tokenizer, max_length, example):
    pass

def get_input_4(tokenizer, max_length, example):
    max_segments = 4
    passage = example.passage
    question = example.question
    options = example.options
    ...
    for index in range(max_segments):
        encoded_option_evidence = tokenizer(question + '[SEP]' + options[index] + '[SEP]' + options_evidence[index],
                                            padding = "max_length",
                                            max_length = max_length,
                                            truncation=True,
                                            add_special_tokens=True)
        if(len(encoded_option_evidence['input_ids']) != max_length):
            print(len(encoded_option_evidence['input_ids']))
        segments.append(encoded_option_evidence['input_ids'])
        segments.append(encoded_option_evidence['attention_mask'])

    return (segments)

def get_input_5(tokenizer, max_length, example):
    pass
...
def segment_init(tokenizer, max_length, input_type, example):
    ...
    options = example.options
    # 先将question和options进行编码
    max_segments = int(input_type.split('_')[0])
    encoded_question_options = tokenizer(question + '[SEP]' + '[SEP]'.join(options), add_special_tokens=True)
    segments = []
...
def prepare_input(tokenizer, max_length, input_type, example):
    if input_type == '4':
        return get_input_4(tokenizer, max_length, example)


def prepare_input_bak(tokenizer, max_length, input_type, example):
    passage = example.passage
...
def process_batch(config, batch_data):
    ...
    for example in batch_data:
        ...
        #cid = torch.tensor([example.cid])
        #answer = torch.tensor([example.answer])
