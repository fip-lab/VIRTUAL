...
def evaluate(config, model, dev_dataloader, test=False, return_predictions=False):
    model.eval()
    loss_total = 0
    predict_all = np.array([], dtype=int)
    labels_all = np.array([], dtype=int)
    cids_all = np.array([], dtype=int)
    ...
    with torch.no_grad():
        for batch in dev_dataloader:
            ...
            cids = cids.data.cpu().numpy()
            
            labels_all = np.append(labels_all, labels)
            predict_all = np.append(predict_all, predic)
            cids_all = np.append(cids_all, cids)
    ...
    if test:
        ...
        if return_predictions:
            return acc, loss_total / len(dev_dataloader), report, confusion, predict_all, labels_all, cids_all
